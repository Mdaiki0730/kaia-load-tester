...
func SetStrictNonceCheck(sn bool) {
	strictNonceCheck = sn
}
...
func GetAccountFromKey(id int, key string) *Account {
	...
	tAcc := Account{
		...
		sync.Mutex{},
		0,
		false, // The account which is generated by GetAccountFromKey should not be checked strictly.
	}
...
func NewLocalReservoirAccount() *Account {
	acc, err := crypto.GenerateKey()
	if err != nil {
		log.Fatalf("crypto.GenerateKey() : Failed to generateKey %v", err)
	}
...
func (acc *Account) GetNonce(c *client.Client) uint64 {
	if acc.nonce != 0 && !acc.strictNonceCheck {
		return acc.nonce
	}
	ctx := context.Background()
